...
def test_phase_a_shapes_and_gradients():
    ...
    variables = model.init(jax.random.key(6), representation)
    change, action = model.apply(variables, representation)
    assert change.shape == (2, 16, 16)
    assert action.shape == target.shape

    def loss(params):
        _, prediction = model.apply({"params": params}, representation)
