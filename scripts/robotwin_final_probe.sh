...
wait_for_finish() {
  ...
  #
  # Anchor the patterns. `pgrep -f` scans whole command lines, so a leftover
  # launcher shell whose text merely mentions "scripts/train.py" (a debug echo,
  # say) matches forever and pins this loop open - that happened tonight and
  # would have stopped arms C and D from ever starting. Anchoring also stops the
  # shell running *this* check from matching itself.
