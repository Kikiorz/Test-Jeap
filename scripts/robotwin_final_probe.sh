#!/usr/bin/env bash
# Probe the final checkpoint of both RoboTwin arms once training has stopped.
#
...
# numeric directory is the converged pair we want to compare.
set -euo pipefail
...
echo "[$(date -u +%H:%M:%S)] training stopped; final steps A=$A_STEP B=$B_STEP" | tee -a "$LOG"

# The two arms must be compared at the same step, otherwise the probe is not paired.
STEP="$A_STEP"
...
label = {"base": "base (Con1 silenced)", "a": "arm A (Con1)", "b": "arm B (Con1+Con2+ctx)"}
flows = {}
print("=" * 68)
for name in ("base", "a", "b"):
...
for left, right in (("a", "base"), ("b", "base"), ("b", "a")):
