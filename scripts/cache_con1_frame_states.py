#!/usr/bin/env python3
"""Cache independent raw frozen V-JEPA2.1 states for anchored Con1.

Phi(o) = concat_views(mean_spatial(VJEPA([o, o]))).  Neither a future frame
nor another episode enters Phi(o).  The head loader constructs raw future-minus-current displacements for H=10.
"""
...
def make_contract(args, info, episodes):
    ...
    return {
        ...
        "normalization": "none; raw float16 states",
        "delta": "z[t+j] - z[t], j=1..10",
...
def state_diagnostics(states):
    ...
    for horizon in (1, 5, 10):
        if len(values) <= horizon:
            continue
        anchor, future = values[:-horizon], values[horizon:]
        cosine = np.sum(anchor * future, axis=-1) / (norm[:-horizon] * norm[horizon:])
        delta = future - anchor
        result[f"h{horizon}"] = {
            "count": len(delta),
