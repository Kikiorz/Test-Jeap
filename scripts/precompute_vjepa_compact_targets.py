...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--projection-file",
        type=Path,
        help="Optional .npy projection generated by JAX; useful when the CUDA teacher environment has no JAX.",
    )
...
def fixed_projection(dim: int, seed: int, path: Path | None) -> np.ndarray:
    if path is not None:
        ...
        if value.shape != (TEACHER_DIM, dim) or value.dtype != np.float32:
            raise ValueError(f"Invalid projection {path}: shape={value.shape}, dtype={value.dtype}")
        return value
    import jax
...
def main() -> None:
    ...
    projection = fixed_projection(args.compact_dim, args.projection_seed, args.projection_file)
    contract = manifest(args, info, episodes, projection)
    if args.plan_only:
